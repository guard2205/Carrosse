...
namespace Carrosse
{
    class MyLegSkieur : MonRectangleTournant
    {
        #region Données Membres
        private MonRectangleTournant  _Thigh;          // calf= mollet   thight =Cuisse 
        private MyFoot _Foot;
        #endregion

        #region Constructeur
        public MyLegSkieur(PictureBox hebergeur, int xsg, int ysg, int longueur, int hauteur) :base(hebergeur,xsg, ysg, longueur, hauteur)
        {
            this._Thigh = new MonRectangleTournant(hebergeur, xsg, ysg + hauteur, longueur, hauteur);
            this._Foot = new MyFoot(hebergeur, xsg, ysg + 2*hauteur, (int)(longueur * 3 / 2), hauteur / 3);
